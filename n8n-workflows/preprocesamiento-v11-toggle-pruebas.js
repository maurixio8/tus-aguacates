// =====================================================
// 🧠 PRE-PROCESAMIENTO v11 - CON TOGGLE ADMIN/CLIENTE
// =====================================================
...
// =====================================================
// 🔄 TOGGLE: COMANDOS /test y /admin
// =====================================================
const msgLowerTrim = messageText.toLowerCase().trim();
const esComandoTest = msgLowerTrim === '/test';
const esComandoAdmin = msgLowerTrim === '/admin';
    ...
    if (esComandoTest) {
        ...
        return [{
            json: {
                ...
                messageText: 'Hola',  // Simular saludo de cliente
                ...
                _notaInterna: '⚡ Modo CLIENTE activado para pruebas'
            }
        }];
    } else if (esComandoAdmin) {
        // El director vuelve a modo admin/copiloto
        return [{
            json: {
                esComandoCopiloto: true,
                from, to, customerName, saludo,
                messageText: 'Modo Admin Activado',
                modoAdminActivado: true,
                telefono_objetivo: null,
                nombre_extraido: null,
                esMediaNoSoportado: false,
                esMedia: false,
                tipoMensaje: messageType,
                timestamp: whatsappMsg?.sendTime,
                _notaInterna: '🔑 Modo COPILOTO activado'
            }
        }];
    }

    // Sin comando de toggle → Comportamiento normal como Copiloto
    const phoneMatch = messageText.replace(/[^\d]/g, '').match(/(?:57)?(3\d{9})/);
