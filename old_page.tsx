...
interface ProductGrouped {
  ...
  // Unidades f├¡sicas totales (considerando multiplicadores de variante)
  // Ej: 1x "2 Bandejas" + 4x "1 Bandeja" = 5 unidades vendidas pero 6 bandejas f├¡sicas
  total_physical_units?: number;
  // Nombre de la unidad f├¡sica (ej: "Bandeja", "unidad")
  ...
  // Peso de la unidad m├ís peque├▒a encontrada (en gramos)
  smallest_weight_grams?: number;
  // Total convertido a unidades de la presentaci├│n m├ís peque├▒a
  total_in_smallest_units?: number;
  // Indica si hay clientes sin variante definida
  has_missing_variants: boolean;
  // En cu├íntos pedidos aparece
  orders_count: number;
...
export default function ListaComprasPage() {
  ...
  // Basado en informaci├│n del usuario:
  // - Combo Ahorro #1: 1kg fresas premium
  // - Combo Ahorro #2: Caja 24 aguacates + Ar├índanos 250g
  // - Combo Ahorro #3: Fresas econ├│mica 500g + Ar├índanos 250g + Paquete 4 aguacates injerto
  // - Combo Mercado Semanal: Caja 24 aguacates, Fresa econ├│mica, Banano 1kg, Tomate 500g, Cebolla 500g, etc.
  const COMBO_COMPONENTS: Record<string, Array<{ name: string; quantity: number; unit: string; variant?: string }>> = {
    ...
    'combo ahorro #2': [
      ...
      { name: 'Ar├índanos Org├ínicos', quantity: 1, unit: 'paq', variant: 'X250grs' }
    ],
    'combo ahorro #3': [
      { name: 'Fresa Econ├│mica', quantity: 1, unit: 'paq', variant: '500grs' },
      { name: 'Ar├índanos Org├ínicos', quantity: 1, unit: 'paq', variant: 'X250grs' },
    ...
    'combo mercado semanal completo': [
      ...
      { name: 'Ar├índanos Org├ínicos', quantity: 1, unit: 'paq', variant: 'X125grs' },
      { name: 'Uva isabelina', quantity: 1, unit: 'bandeja', variant: '400grs' },
      { name: 'Duraznos', quantity: 1, unit: 'lb', variant: '500 gr' },
      { name: 'Lim├│n Tahiti', quantity: 1, unit: 'kg', variant: '1000 gr' }
    ],
    'combo navide├▒o premium': [
  ...
  // Inicializar con ├║ltimos 10 d├¡as
  useEffect(() => {
    const today = new Date();
  ...
  const duplicateWarnings = useMemo(() => {
    ...
    // 1. Agrupar por Tel├®fono (muy fuerte indicador)
    ...
    byPhone.forEach((group, phone) => {
      if (group.length > 1) {
        ...
        warnings.push(`Mismo tel├®fono (${phone}): ${names} (${group.length} pedidos)`);
        group.forEach(o => processedIds.add(o.id));
      }
    });

    // 2. Agrupar por Nombre Normalizado (si no fueron capturados por tel├®fono)
    const byName = new Map<string, Order[]>();
    activeOrders.forEach(o => {
      // Si ya fue procesado por tel├®fono, ignorar para evitar duplicar alertas
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Resetear selecci├│n cuando cambian los pedidos
  ...
  // Detectar categor├¡a del producto bas├índose en palabras clave del nombre
  ...
  const getCategoryStyle = (productName: string): { bg: string; border: string; label: string; color: string } => {
    ...
    // Frutas
    const frutas = ['manzana', 'fresa', 'ar├índano', 'arandano', 'naranja', 'lim├│n', 'limon', 'mandarina',
      'mango', 'pi├▒a', 'pina', 'papaya', 'banano', 'banana', 'uva', 'ciruela', 'durazno',
      'mora', 'granadilla', 'maracuy├í', 'maracuya', 'guayaba', 'aguacate', 'pera', 'sand├¡a',
      'sandia', 'mel├│n', 'melon', 'cereza', 'kiwi', 'coco', 'pitiahaya', 'pitaya', 'lulo', 'tomate de ├írbol'];
    ...
    // Verduras y Hierbas
    const verduras = ['lechuga', 'espinaca', 'kale', 'r├║gula', 'rugula', 'acelga', 'apio', 'br├│coli',
      'brocoli', 'coliflor', 'zanahoria', 'pepino', 'tomate', 'cebolla', 'ajo', 'cilantro',
      'perejil', 'albahaca', 'hierbabuena', 'menta', 'romero', 'tomillo', 'or├®gano', 'oregano',
      'papa', 'yuca', 'pl├ítano', 'platano', 'ma├¡z', 'maiz', 'arveja', 'habichuela',
      'calabac├¡n', 'calabacin', 'berenjena', 'piment├│n', 'pimenton', 'champi├▒├│n', 'champi├▒on'];
    ...
    // Default - sin categor├¡a espec├¡fica
  ...
  const extractWeightFromVariant = (variantName: string | null): number | undefined => {
    ...
    // Patrones de b├║squeda (en orden de especificidad)
    const patterns = [
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        // Si est├í en kg, convertir a gramos
  ...
  // Elimina variantes o n├║meros repetidos al final del nombre
  const normalizeProductName = (name: string, variant?: string | null): string => {
    if (!name) return 'Producto sin nombre';
    let normalized = name.trim();

    // SIEMPRE remover CUALQUIER contenido final entre par├®ntesis
  ...
  // Crear clave de agrupaci├│n inteligente
  ...
  const createGroupingKey = (productName: string, variant: string | null): string => {
    // Usar SOLO el nombre normalizado como clave de agrupaci├│n
    // Esto garantiza que todas las variantes del mismo producto se agrupen
    return normalizeProductName(productName, variant);
  };

  // Detectar si el nombre del producto ya contiene informaci├│n de cantidad/tama├▒o
  // Ej: "Caja de 12 unidades Premium" -> true
  // Ej: "Ar├índanos Org├ínicos" -> false
  ...
  // Extraer multiplicador de la variante para calcular unidades f├¡sicas
  // Ej: "2 Bandejas" -> 2, "3 unidades" -> 3, "X2" -> 2
  // EXCEPCI├ôN: Para productos tipo "Caja de X unidades", NO multiplicar porque
  ...
  const groupedProducts = useMemo(() => {
    ...
    selectedOrdersList.forEach(order => {
      ...
      items.forEach(item => {
        ...
        if (comboComponents) {
          ...
          // variant_value contiene el valor espec├¡fico (ej: "X125grs", "X250grs")
          // variant_name contiene el tipo gen├®rico (ej: "Presentaci├│n", "Tama├▒o")
          ...
          // Crear clave de agrupaci├│n inteligente (detecta si variante ya est├í en el nombre)
          ...
          if (productMap.has(groupingKey)) {
            ...
            // Calcular unidades f├¡sicas para este item (SIEMPRE, no solo con multiplicador > 1)
            ...
            // Detectar nombre de unidad f├¡sica si no existe
            ...
            if (itemWeightGrams) {
              ...
              // Actualizar peso m├¡nimo si este es menor
              ...
              // Recalcular unidades equivalentes en la presentaci├│n m├ís peque├▒a
            ...
            // Calcular unidades f├¡sicas (considerando multiplicador de variante)
            // Ej: 1 pedido de "2 Bandejas" = 2 bandejas f├¡sicas
            const multiplier = extractMultiplierFromVariant(variantDisplay, normalizedName);
            ...
            productMap.set(groupingKey, {
              ...
              // Siempre guardar unidades f├¡sicas si hay nombre de unidad (bandeja, etc.)
    ...
    return Array.from(productMap.values()).sort(
      (a, b) => {
        // Orden de prioridad de categor├¡as (1: Combo/Caja, 2: Fruta, 3: Verdura, 4: Otro)
        ...
        // 1. Clasificar por Categor├¡a
        if (priorityA !== priorityB) {
          return priorityA - priorityB;
        }

        // 2. Si es la misma categor├¡a, ordenar Alfab├®ticamente por nombre
  ...
  const salesSummary = useMemo(() => {
    ...
    selectedOrdersList.forEach(order => {
      ...
      // Si no hay subtotal expl├¡cito, calcularlo desde los items del pedido
      if (!productAmount && orderTotal > 0) {
        ...
        if (items.length > 0) {
          ...
          // Como ├║ltimo recurso, restar el env├¡o del total
      ...
      // El total general es Productos + Env├¡os (para consistencia)
  ...
  const generateOrderSummary = (customer: CustomerBreakdown) => {
    ...
    customer.order_items.forEach(item => {
      ...
      lines.push(`ÔÇó ${item.product_name}${variant}: ${item.quantity} unidad${item.quantity === 1 ? '' : 'es'}${weight}`);
    });

    if (customer.customer_address) {
      lines.push('', `Direcci├│n: ${customer.customer_address}`);
    }
  ...
  const exportToExcel = () => {
    ...
    const BOM = '\uFEFF';
    const headers = ['Producto', 'Variante', 'Cantidad Total', 'Peso Total', 'Cliente', 'Direcci├│n', 'Cantidad Cliente', 'Peso Cliente'];
                ...
                <div className="bg-amber-50 border-b border-amber-200 p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amber-700 mb-2">Revisa si estos pedidos pertenecen al mismo cliente para unificar el env├¡o:</p>
                              ...
                              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                                ...
                                {product.has_missing_variants && (
                                  <p className="text-xs text-amber-600 font-medium bg-amber-50 px-2 py-0.5 rounded">
                                    ÔÜá´©Å Hay pedidos sin variante
                                  </p>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="text-right flex-shrink-0">
                            ...
                            {product.total_weight_display ? (
                              <>
                                ...
                                {/* Mostrar unidades equivalentes en presentaci├│n m├¡nima si es diferente */}
                                {product.total_in_smallest_units && product.smallest_weight_grams && (
                                  <p className="text-sm text-gray-600 font-medium">
                                    ({product.total_in_smallest_units} ├ù {formatWeight(product.smallest_weight_grams)})
                                  </p>
                                )}
                              </>
                            ) : product.total_physical_units && product.total_physical_units > product.total_quantity ? (
                              /* Mostrar unidades f├¡sicas como n├║mero principal cuando son diferentes */
                              <>
                                <p className="text-2xl font-bold text-green-600">
                              ...
                              return (
                                ...
                                >
                                      ...
                                      {customer.variant_name || customer.weight_display ? (
                                        ...
                                        <>
                                          <p className="font-bold text-amber-600 text-lg">
                                            {customer.quantity}x ÔÜá´©Å Sin dato
                                          </p>
                                  ...
                                  <div className="flex gap-2 mt-2 ml-11">
                                    ...
                                    {/* Bot├│n para ver el pedido completo */}
                                    <a
