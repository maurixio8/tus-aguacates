...
const replacement = [
  ...
  "      'hass':'\u{1F951}','injerto':'\u{1F951}','caja de 24':'\u{1F951}','caja de 12':'\u{1F951}',",
  "      'caja de 7':'\u{1F951}','caja de 35':'\u{1F951}','nueva maya':'\u{1F951}',",
  "      // Uvas espec\u00edficas",
  "      'red globe':'\u{1F347}',",
  "      // Frutas",
...
// Find current lines 334-369 (the old EMOJI_TABLE + getProductEmoji)
const startIdx = lines.findIndex(l => l.includes('Tabla Maestra de emojis'));
const endIdx = lines.findIndex((l, i) => i > startIdx && l.trim().startsWith('};') && lines[i-1]?.includes('📦'));
...
if (startIdx >= 0 && endIdx >= 0) {
  ...
  console.log('Done! Replaced', (endIdx - startIdx + 1), 'lines with', replacement.length, 'new lines');
} else {
