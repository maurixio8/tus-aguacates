...
try:
    ...
    print("Error: Se requiere la librería 'requests'. Instala con: pip install requests")
    sys.exit(1)
...
class N8NManager:
    ...
    def run_webhook(self, webhook_url: str, data: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    def audit_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        for node in nodes:
            ...
            if any(ai in node_type.lower() for ai in ["openai", "langchain", "agent", "chat"]):
                audit["ai_nodes"].append({"name": node_name, "type": node_type})
            
            # Identificar nodos de base de datos
            if any(db in node_type.lower() for db in ["postgres", "mysql", "supabase", "mongodb"]):
        ...
        if len(audit["triggers"]) == 0:
            audit["warnings"].append("⚠️ No se encontraron triggers. El workflow no puede iniciar automáticamente.")
        
        if len(audit["triggers"]) > 1:
            audit["warnings"].append(f"⚠️ Se encontraron {len(audit['triggers'])} triggers. Solo uno puede estar activo.")
        
        if len(audit["ai_nodes"]) > 0 and len(audit["database_nodes"]) == 0:
            audit["recommendations"].append("💡 Considera agregar persistencia para conversaciones con nodos de base de datos.")
        
        if len(audit["code_nodes"]) > 3:
            audit["recommendations"].append("💡 Muchos nodos de código. Considera modularizar en sub-workflows.")
...
def main():
    ...
    # Comando: create
    create_parser = subparsers.add_parser("create", help="Crea un workflow desde archivo JSON")
    create_parser.add_argument("file", help="Archivo JSON del workflow")
    
    # Comando: update
    update_parser = subparsers.add_parser("update", help="Actualiza un workflow existente")
    ...
    # Comando: deactivate
    deactivate_parser = subparsers.add_parser("deactivate", help="Desactiva un workflow")
    ...
    # Comando: run_webhook
    webhook_parser = subparsers.add_parser("run_webhook", help="Ejecuta un workflow vía webhook")
    webhook_parser.add_argument("url", help="URL del webhook")
    webhook_parser.add_argument("--data", "-d", help="Datos JSON a enviar", default="{}")
    
    # Comando: audit
    audit_parser = subparsers.add_parser("audit", help="Audita la lógica de un workflow")
    audit_parser.add_argument("id", help="ID del workflow")
    
    # Comando: export
    export_parser = subparsers.add_parser("export", help="Exporta un workflow a archivo")
