...
export async function POST(request: NextRequest) {
  try {
    console.log('🔑 [Admin-Login API] Recibida petición de login');

    const body = await request.json();
    const { email, password } = body;

    console.log('🔑 [Admin-Login API] Intento de login:', {
      email,
    ...
    // VALIDACIÓN SIMPLE
    if (!email || !password) {
      console.warn('⚠️  [Admin-Login API] Faltan email o password en request');
    ...
    if (email === 'admin@tusaguacates.com' && password === '7FdX9Zq-hson&j39') {
      console.log('✅ [Admin-Login API] Credenciales válidas - Admin verificado');
      ...
      console.log('🔐 [Admin-Login API] JWT creado exitosamente:', {
        userId: adminUser.id,
        expiresIn: '24 horas',
        claims: {
          id: adminUser.id,
    ...
    console.warn('⚠️  [Admin-Login API] Credenciales incorrectas:', {
      email,
      timestamp: new Date().toISOString()
    });
    ...
    console.error('❌ [Admin-Login API] Error en login:', {
