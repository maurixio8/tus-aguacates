...
export async function POST(request: NextRequest) {
  try {
    console.log('🔑 [Admin-Signin API] Recibida petición de login');

    const body = await request.json();
    const { email, password } = body;

    console.log('🔑 [Admin-Signin API] Intento de login:', {
      email,
    ...
    // VALIDACIÓN SIMPLE
    if (!email || !password) {
      console.warn('⚠️  [Admin-Signin API] Faltan email o password en request');
    ...
    if (email === 'admin@tusaguacates.com' && password === '7FdX9Zq-hson&j39') {
      console.log('✅ [Admin-Signin API] Credenciales válidas - Admin verificado');
      ...
      console.log('🔐 [Admin-Signin API] JWT creado exitosamente:', {
        userId: adminUser.id,
        expiresIn: '24 horas',
        claims: {
          id: adminUser.id,
      ...
      console.log('✅ [Admin-Signin API] Login exitoso - Usuario:', {
        id: adminUser.id,
        email: adminUser.email,
        role: adminUser.role
      });
    ...
    console.warn('⚠️  [Admin-Signin API] Credenciales incorrectas:', {
      email,
      timestamp: new Date().toISOString()
    });
    ...
    console.error('❌ [Admin-Signin API] Error en login:', {
