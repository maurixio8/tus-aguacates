-- ============================================================================
-- MIGRACIÓN: Crear tablas B2B para sección de empresas (VERSÓN SEGURA)
-- Fecha: 2025-01-28
...
-- Primero ejecutar limpieza si necesitas empezar de cero
-- \i cleanup_b2b.sql
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_b2b_categories_updated_at'
    AND tgrelid = 'b2b_categories'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_companies_updated_at'
    AND tgrelid = 'b2b_companies'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_company_users_updated_at'
    AND tgrelid = 'b2b_company_users'::regclass
  ) THEN
    CREATE TRIGGER update_b2b_company_users_updated_at
    BEFORE UPDATE ON b2b_company_users
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_products_updated_at'
    AND tgrelid = 'b2b_products'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_pricing_tiers_updated_at'
    AND tgrelid = 'b2b_pricing_tiers'::regclass
  ) THEN
    CREATE TRIGGER update_b2b_pricing_tiers_updated_at
    BEFORE UPDATE ON b2b_pricing_tiers
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_orders_updated_at'
    AND tgrelid = 'b2b_orders'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'update_b2b_recurring_orders_updated_at'
    AND tgrelid = 'b2b_recurring_orders'::regclass
  ) THEN
    CREATE TRIGGER update_b2b_recurring_orders_updated_at
    BEFORE UPDATE ON b2b_recurring_orders
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
...
-- Habilitar RLS en todas las tablas (es seguro hacerlo múltiples veces)
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Todos pueden ver categorías B2B activas"
  ON b2b_categories FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Solo admins pueden gestionar categorías B2B"
...
-- RLS: b2b_companies
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Usuarios pueden ver su propia empresa"
...
CREATE POLICY IF NOT EXISTS "Admins pueden ver todas las empresas B2B"
  ON b2b_companies FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
CREATE POLICY IF NOT EXISTS "Usuarios pueden crear su empresa"
  ON b2b_companies FOR INSERT WITH CHECK (created_by_user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Usuarios pueden actualizar su empresa"
...
CREATE POLICY IF NOT EXISTS "Admins pueden gestionar empresas B2B"
...
CREATE POLICY IF NOT EXISTS "Admins pueden ver todos los usuarios B2B"
  ON b2b_company_users FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
CREATE POLICY IF NOT EXISTS "Usuarios pueden invitarse a su empresa"
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Todos pueden ver productos B2B activos"
  ON b2b_products FOR SELECT USING (is_active = true AND deleted_at IS NULL);

CREATE POLICY IF NOT EXISTS "Solo admins pueden gestionar productos B2B"
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Todos pueden ver pricing tiers B2B"
  ON b2b_pricing_tiers FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Solo admins pueden gestionar pricing tiers B2B"
...
-- RLS: b2b_orders
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Usuarios pueden ver sus pedidos B2B"
...
CREATE POLICY IF NOT EXISTS "Admins pueden ver todos los pedidos B2B"
  ON b2b_orders FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
CREATE POLICY IF NOT EXISTS "Usuarios pueden crear pedidos B2B"
  ON b2b_orders FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins pueden gestionar pedidos B2B"
...
-- RLS: b2b_order_items
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Usuarios pueden ver items de sus pedidos B2B"
...
CREATE POLICY IF NOT EXISTS "Admins pueden ver todos los items B2B"
  ON b2b_order_items FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
-- ============================================================================
CREATE POLICY IF NOT EXISTS "Usuarios pueden ver pedidos recurrentes de su empresa"
...
CREATE POLICY IF NOT EXISTS "Admins pueden ver todos los pedidos recurrentes B2B"
  ON b2b_recurring_orders FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
CREATE POLICY IF NOT EXISTS "Usuarios pueden crear pedidos recurrentes B2B"
  ON b2b_recurring_orders FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Usuarios pueden actualizar sus pedidos recurrentes B2B"
  ON b2b_recurring_orders FOR UPDATE USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins pueden gestionar pedidos recurrentes B2B"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'set_b2b_order_number_trigger'
    AND tgrelid = 'b2b_orders'::regclass
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'validate_b2b_pricing_tiers_overlap'
    AND tgrelid = 'b2b_pricing_tiers'::regclass
  ) THEN
