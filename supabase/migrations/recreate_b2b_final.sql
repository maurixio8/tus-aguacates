-- ============================================================================
-- MIGRACIÓN B2B FINAL - VERSIÓN ROBUSTA
-- Ejecutar este script COMPLETO en el SQL Editor de Supabase
-- ============================================================================

-- ============================================================================
-- PARTE 1: LIMPIEZA CON DO BLOCKS (maneja errores graciosamente)
-- ============================================================================
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Triggers cleanup completed with warnings';
END $$;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Functions cleanup completed with warnings';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_categories
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_categories policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_companies
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_companies policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_company_users
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_company_users policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_products
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_products policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_pricing_tiers
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_pricing_tiers policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_orders
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_orders policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_order_items
    DROP POLICY IF EXISTS "Todos pueden ver items de pedidos B2B" ON b2b_order_items;
    DROP POLICY IF EXISTS "Admins pueden ver todos los items B2B" ON b2b_order_items;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_order_items policies cleanup completed';
END $$;

DO $$
BEGIN
    -- Eliminar políticas RLS en b2b_recurring_orders
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'b2b_recurring_orders policies cleanup completed';
END $$;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'RLS disable completed with warnings';
END $$;
...
CREATE TRIGGER update_b2b_categories_updated_at BEFORE UPDATE ON b2b_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_companies_updated_at BEFORE UPDATE ON b2b_companies FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_company_users_updated_at BEFORE UPDATE ON b2b_company_users FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_products_updated_at BEFORE UPDATE ON b2b_products FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_pricing_tiers_updated_at BEFORE UPDATE ON b2b_pricing_tiers FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_orders_updated_at BEFORE UPDATE ON b2b_orders FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_b2b_recurring_orders_updated_at BEFORE UPDATE ON b2b_recurring_orders FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Usuarios pueden ver usuarios de su empresa" ON b2b_company_users FOR SELECT USING (EXISTS (SELECT 1 FROM b2b_company_users AS cu WHERE cu.company_id = b2b_company_users.company_id AND cu.user_id = auth.uid()));
CREATE POLICY "Admins pueden ver todos los usuarios B2B" ON b2b_company_users FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));
CREATE POLICY "Usuarios pueden invitarse a su empresa" ON b2b_company_users FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM b2b_company_users WHERE b2b_company_users.company_id = b2b_company_users.company_id AND b2b_company_users.user_id = auth.uid() AND b2b_company_users.role IN ('owner', 'admin')));
...
-- ============================================================================
-- PARTE 5: FUNCIONES Y TRIGGERS ADICIONALES
-- ============================================================================
...
CREATE TRIGGER set_b2b_order_number_trigger BEFORE INSERT ON b2b_orders FOR EACH ROW EXECUTE FUNCTION set_b2b_order_number();
...
CREATE TRIGGER validate_b2b_pricing_tiers_overlap BEFORE INSERT OR UPDATE ON b2b_pricing_tiers FOR EACH ROW EXECUTE FUNCTION validate_b2b_pricing_tiers_no_overlap();
